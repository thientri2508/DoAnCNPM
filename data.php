<?php
	...
	if (isset($_POST['getData'])&&$_POST['start']>=0) {
		$sql="select * from order_ticket LIMIT ".$_POST['start'].", ".$_POST['limit'];
		$order=pdo_query($sql);
		...
		if (count($order) > 0){
			...
			$slsp=pdo_query($sql);
			$response.= '<div style="width:100%; height:280px; margin-top:30px; border-bottom: solid 3px;border-top : solid 3px;" id="DH-'.$id_order.'"> 
				<div style="float:left; height:100%; width:30%; margin-left:20px">
				 ...
				 <div style="float:left; height:100%; width:40%; margin-left:20px">
					...
					if($id_promotions==0) $response.='<div class="DS_TT1">không áp dụng</div>';
					else {
					    $sql="select name from promotions where id=".$id_promotions;
					    $promotions_name=pdo_query_one($sql);
					    $response.='<div class="DS_TT1">'.$promotions_name["name"].'</div>';
					}
				$response.= '<div class="DS_TT" style="font-size:18px"><b>Tổng Tiền</b></div>
				...
				<div style="float:right; height:100%; width:20%">
					...
					if($status=='chờ xác nhận') $response.= '<button type="button" class="btn btn-warning" style="margin-top:50px; height:60px; width:120px" onclick="xacnhanDH('.$id_order.')"><b>Xác Nhận</b></button>';
					else $response.= '<img src="photo/checked.png" style="margin-top:25px; height:100px; width:100px; margin-left:10px" />';
				$response.= '</div>
				</div>';
			}
			exit($response);
