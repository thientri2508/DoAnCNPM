...
<?php
	if(isset($_GET['ID'])){
		...
		$sql="select * from set_promotions where id_promotions=".$_GET['ID'];
		$ds=pdo_query($sql);
	}else{
...
<h2 style="font-feature-settings: 'kern'; width:70%; margin:auto; text-align:center; margin-top:50px"><b>CHỈNH SỬA CHƯƠNG TRÌNH KHUYẾN MÃI</b></h2>
...
<div style="width:45%; float:left; height:100%; background:#fff; margin-left:40px; position:relative">
	<form method="post" action="xulifixCTKM.php">
    <input type="hidden" name="id" value="<?php echo $promotions['id'] ?>">
	<h5 style="margin-left:40px; margin-top:30px">Tên Chương Trình</h5>
    <input type="text" class="form-control" style="width:75%; margin-left:40px"  name="namect" id="namect" value="<?php echo $promotions['name'] ?>">
    ...
    <input type="date" value="<?php echo $promotions['date_begin'] ?>" class="form-control" style="width:75%; margin-left:40px"  name="start" id="start">
    ...
    <input type="date" value="<?php echo $promotions['date_end'] ?>" class="form-control" style="width:75%; margin-left:40px"  name="end" id="end">
...
<div style="width:45%; float:left; height:100%; background:#fff; margin-left:40px">
	...
	foreach($content as $c){
		...
		if($flag){
			...
			echo '  <div class="form-check" style="margin-left:20px; margin-top:20px">
					<input class="form-check-input" type="checkbox" name="content-'.$c["id"].'" checked>
					<label class="form-check-label" for="flexCheckDefault">Giảm '.$c["price_sale"].' VNĐ cho hóa đơn tối thiểu '.$c["apply"].' VNĐ</label>
					</div> ';
		}	
	}
...
<input type="submit" class="btn btn-outline-success" style="float:left; height:100%; width:25%; font-size:22px; border:solid 3px" value="Cập Nhật" onclick="return checkCTKM();"></input>
